...
class RegistrationController extends AbstractController
{
    ...
    public function __construct(EmailVerifier $emailVerifier, AlertServiceInterface $alertService)
    {
    ...
    public function register(Request $request, UserPasswordHasherInterface $userPasswordHasher, UserAuthenticatorInterface $userAuthenticator, AppCustomAuthenticator $authenticator, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->emailVerifier->sendEmailConfirmation('app_verify_email', $user,
                (new TemplatedEmail())
                    ->from(new Address('floriane@gmail.com', 'floriane'))
                    ->to($user->getEmail())
                    ->subject('Please Confirm your Email')
                    ->htmlTemplate('registration/confirmation_email.html.twig')
            );
            // do anything else you need here, like send an email
            $this->alertService->success('Un email vous a été envoyé pour verifier votre email !');
    ...
    public function verifyUserEmail(Request $request, UserRepository $userRepository): Response
    {
        ...
        $this->alertService->success('Votre Adresse email a été verifié !');
